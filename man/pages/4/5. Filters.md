`dices` provide four filter operators. They take a list on their left side, and
remove or keep the number of elements requested by their right side.
...
They can be used in conjunction with `d` to express what in tabletop gaming is
called *throwing with (dis)advantage*.
